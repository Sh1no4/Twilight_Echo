...
import { deletePluginSetting, getPluginSetting, setPluginSetting } from './settingsStore.ts'
import { initProxy } from './proxyBootstrap.ts'
...
/**
 * Transport adapter for the plugin host core.
 *
 * Electron runs the host inside a utility process and delivers messages over
 * `parentPort` (structured-clone IPC). Tauri runs the same host as a Node
 * sidecar and delivers messages over stdin/stdout as JSON lines. Both adapters
 * expose this small surface; the host core (`runPluginHost`) is transport-free.
 */
export interface PluginHostTransport {
  ...
  onMessage: (listener: (message: PluginHostRequest | PluginHostApiResult) => void) => () => void
  /** Register a disconnect listener (child died / parent gone); returns an unsubscribe function. */
...
/**
 * Runs the plugin host core against a transport adapter.
 *
 * This is the shared implementation used by the Electron utility-process host
 * (`src/main/pluginHost.ts`) and by the Tauri Node sidecar
 * (`src/main/plugins/pluginHostNode.ts`). The host owns one active plugin at a
 * time; the parent drives it with `activate` / `deactivate` / `event` /
 * `provider-call` / `ui-command` / `cancel` messages and answers `api-call`
 * messages with `api-result`.
 */
export function runPluginHost(transport: PluginHostTransport): void {
  ...
  // If the parent disappears, stop accepting new work. Deactivation cleanup
  // happens on the parent side when it tears the process down.
  transport.onDisconnect?.(() => {
    abortPendingPluginCalls('Plugin host transport disconnected.')
    rejectPendingApiCalls('Plugin host transport disconnected.')
    unsubscribe()
  })
