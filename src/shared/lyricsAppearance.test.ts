...
test('lyrics appearance migrates legacy lyric style settings into independent layers', () => {
  ...
  assert.equal(normalized.styles.active.fontSize, 31)
  assert.equal(normalized.styles.translation.fontSize, 22)
...
test('legacy quick controls stay compatible with the independent style layers', () => {
  ...
  assert.equal(synced.styles.active.fontSize, 29)
  assert.equal(synced.styles.translation.fontSize, 20)
