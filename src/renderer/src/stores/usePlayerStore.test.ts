...
test('audio visualizer iframe controls are wired to the player store', () => {
  ...
  assert.match(visualizerSource, /const SPECTRUM_ATTACK_SECONDS = 0\.028/)
  assert.match(visualizerSource, /const SPECTRUM_DECAY_SECONDS = 0\.075/)
  ...
  assert.match(visualizerSource, /const SPECTRUM_DISPLAY_HEADROOM = 0\.97/)
  assert.match(visualizerSource, /const SPECTRUM_TARGET_PEAK_LEVEL = 0\.95/)
  ...
  assert.match(visualizerSource, /flex: 0 0 250px;/)
  assert.match(visualizerSource, /height: 250px;/)
  ...
  assert.match(
    visualizerSource,
    /const targetHeight = \(val \/ 255\) \* \(height - 15\) \* SPECTRUM_HEIGHT_SCALE/
  )
