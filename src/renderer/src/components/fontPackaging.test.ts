...
test('renderer CSS packages the licensed built-in font style library and excludes legacy families', () => {
  ...
  assert.match(indexHtml, /\/font\/misans\/full\/misans-full\.css/)

  const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '../../../..')
  const fontDir = join(repoRoot, 'resources/font')
  const misansDir = join(fontDir, 'misans')
  const misansFullDir = join(misansDir, 'full')
  ...
  assert.equal(existsSync(join(misansFullDir, 'misans-full.css')), true)
  assert.equal(existsSync(join(misansFullDir, 'README.md')), true)
  ...
  const fullCss = readFileSync(join(misansFullDir, 'misans-full.css'), 'utf8')
  assert.match(fullCss, /font-family: 'MiSans Full'/)
  assert.match(fullCss, /url\('\.\/MiSans-Regular\.woff2'\)/)
  assert.match(fullCss, /url\('\.\/MiSans-Bold\.woff2'\)/)
  assert.match(base, /MiSans Full/)
