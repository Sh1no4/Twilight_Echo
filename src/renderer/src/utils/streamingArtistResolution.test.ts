...
} = (await import(new URL('./streamingArtistResolution.ts', import.meta.url).href)) as typeof import(
  './streamingArtistResolution'
)
...
test('uses exact artist name matches and rejects unrelated search fallbacks', () => {
  ...
  assert.equal(findBestStreamingArtistMatch('躺在7FIV6怀里', [
...
test('linked musician users resolve to matched artist ids before content fetches', async () => {
  ...
  const resolved = await resolveLinkedStreamingArtist(initialArtist, { name: '沙包--' }, async () => ({
    id: 987,
    name: '沙包--',
    picUrl: null
  }))
...
test('linked musician users keep the initial user id only when no artist match exists', async () => {
  const initialArtist = { id: 123456, name: '沙包--', picUrl: 'user.png' }
  const resolved = await resolveLinkedStreamingArtist(initialArtist, { name: '沙包--' }, async () => null)
...
test('linked musician users are not replaced by the first unrelated search result', async () => {
  ...
  const resolved = await resolveLinkedStreamingArtist(initialArtist, { name: '躺在7FIV6怀里' }, async () =>
