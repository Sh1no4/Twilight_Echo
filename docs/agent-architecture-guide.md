...
| `themes/` | 主题归档与库 | `themeArchiveValidation.ts`、`themeLibraryRepository.ts` | `themes:*`（配合 shared/theme.ts） |
...
| `useThemeStore.ts` | 主题 | 与 shared/theme.ts、main/themes 协同 |
...
- 主题：main `themes/` + shared `theme.ts` + renderer `useThemeStore`。
